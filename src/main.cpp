...
int main() {
  ...
  GLFWwindow *window = glfwCreateWindow(
      winWidth, winHeight, "OpenGL Textures: Kazuya Smile", nullptr, nullptr);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Creates identity matrix to perform tranformations on
    glm::mat4 trans = glm::mat4(1.0f);

    /*In 3D Graphics we want to follow the rule T.R.S when applying
    transformations on translation matrices

    1. Translation
    2. Rotation
    3. Scaling

    This ensures that the math done on the object is appiled in the correct
    order which is actually S.R.T
    */
